...
select id, graduation_year, gender from users;
    ...
    with op.batch_alter_table('users') as tbl:
        tbl.add_column(sa.Column('type', sa.String(1), server_default='u'))
    ...
    op.add_column('professors', sa.Column('user_id', sa.Integer, sa.ForeignKey('users.id')))
...
alter table user_role rename constraint uq_student_role_student_id to uq_user_role_student_id;
